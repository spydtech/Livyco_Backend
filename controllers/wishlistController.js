...
export const addToWishlist = async (req, res) => {
  try {
    ...
    // Check if this user already has the property in wishlist
    const existingWishlist = await Wishlist.findOne({ userId, propertyId });
    if (existingWishlist) {
      return res.json({
        success: false,
        message: "Property already in wishlist for this user",
      });
    }
    ...
    if (!property) {
      property = await Property.findOne({
        $or: [{ customId: propertyId }, { propertyCode: propertyId }],
      });
    }
    ...
    // Construct property data snapshot
    const propertyData = {
      name: property.name,
      city: property.city,
      locality: property.locality,
      ...
      rooms: room
        ? {
            _id: room._id,
            roomTypes: room.roomTypes || [],
            floorConfig: room.floorConfig || { selectedRooms: [], floors: [] },
          }
        : null,
    };
